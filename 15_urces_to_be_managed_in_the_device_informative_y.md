...
### 1.5.2.10 Other Resources 其他资源
| Resource 资源 | Parameters 参数 | Default Actor 默认参与者| Change Policy 更改政策| Notes 备注|
| -- | -- | -- | -- | -- |
| Data Synchronization settings<br/> 数据同步设置 | Application Service Access Point, Server Name, Access Point Link, Proxy, Information Link<br/>应用服务接入点，服务器名称，接入点链路，代理，信息链路 | User, Subscriber<br/>用户，订阅者 | W/O A |  |
| Application and Service Profile – List of applications currently installed, along with optional specific attributes<br/>应用程序和服务配置文件 - 当前安装的应用程序列表，以及可选的特定属性 | Upgrade version date,License information,Usage data,In use flag,Resources consumed (e.g. storage), Archived flag<br/>升级版本日期，许可信息，使用数据，在使用标志中，资源消耗（例如存储），存档标志 | MA | W/A | Not all applications are required to possess all attributes, though some small set might be mandatory. Needed to allow authorised parties, such as customer care or a software management server, to determine the current status of the applications authorised party has access right to.<br/> 并非所有应用程序都需要拥有所有属性，虽然一些小集合可能是强制性的。需要允许授权方（例如客户服务或软件管理服务器）确定授权方有权访问的应用程序的当前状态。 |
| Proximity & Social context<br/>接近和社会语境 | Proximity to Physical or Network services, Social Context<br/>接近物理或网络服务，社会语境 | MA | W/A | Social context is a user- centric concept that covers the user’s situation. Examples are being in a meeting (where documents may be made available), restaurant (and it’s meal time), office vs. home (different connectivity or permissions), etc.<br/>社交语境是一种以用户为中心的概念，涵盖了用户的世纪情况。例如在会议（文件可能在这里提供），餐厅（和它的用餐时间），办公室与家庭（不同的连接或权限）等。 |